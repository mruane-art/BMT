...
// Common MLS field mappings (handles various export formats) — used for client-side CSV parsing
...
export default function MLSImport({ onImport }: MLSImportProps) {
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        if (isPDF) {
          // PDFs must be parsed server-side
          const formData = new FormData();
          formData.append('file', file);
          const res = await fetch('/api/parse-mls', { method: 'POST', body: formData });
          if (!res.ok) throw new Error('Server could not parse this PDF');
          ...
          // CSV / TXT — parse in browser with PapaParse
          Papa.parse(file, {
            header: true,
            skipEmptyLines: true,
            complete: (results) => {
        ...
        {status === 'parsing' ? (
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">
              Drag & drop a PDF or CSV export from your MLS
            </p>
            <p className="text-gray-400 text-xs mt-1">
              Supports PDF · CSV · XLS · auto-maps common fields
            </p>
          </div>
        )}
      </div>
